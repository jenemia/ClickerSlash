...
namespace ClikerSlash.Editor
{
    ...
    public static class PrototypeBattleSceneBuilder
    {
            ...
            out CinemachineCamera loadingDockVirtualCamera)
        {
            ...
            var environmentBinder = presentationRoot.AddComponent<BattleEnvironmentBinder>();
            var palletStackPresenter = presentationRoot.AddComponent<LaneCargoPalletStackPresenter>();
            var transferPresenter = presentationRoot.AddComponent<LaneCargoTransferPresenter>();
            ...
            bridge.BindSceneReferences(
                ...
                laneVirtualCamera,
                loadingDockVirtualCamera);
            bridge.BindActorVisualPrefabs(playerPrefab, supportRobotPrefab);
            miniGamePresenter.BindSceneReferences(mainCamera, null, cargoVisualPrefabs);
            EditorUtility.SetDirty(bridge);
            EditorUtility.SetDirty(miniGamePresenter);
            EditorUtility.SetDirty(conveyorPresenter);
            EditorUtility.SetDirty(environmentBinder);
            EditorUtility.SetDirty(palletStackPresenter);
            EditorUtility.SetDirty(transferPresenter);
        }
