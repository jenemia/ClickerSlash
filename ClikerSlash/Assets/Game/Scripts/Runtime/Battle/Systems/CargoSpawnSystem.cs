...
namespace ClikerSlash.Battle
{
    ...
    public partial struct CargoSpawnSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            if (!PrototypeSessionRuntime.TryConsumeNextLaneCargoSpawn(out var spawnEntry))
            {
                return;
            }
            ...
            // authoritative Env가 준비된 세션에서만 팔레트 -> 레일 handoff 시간을 적용합니다.
            var revealDuration = BattleEnvironmentBindingRuntime.IsReady
                ? LaneCargoTransferPresenter.DefaultTransferDurationSeconds
                : 0f;
            ...
            state.EntityManager.AddComponentData(cargoEntity, new CargoPrefabVariant { Value = spawnEntry.PrefabVariantId });
            state.EntityManager.AddComponentData(cargoEntity, new CargoSpawnSequenceId { Value = spawnEntry.SpawnIndex });
            state.EntityManager.AddComponentData(cargoEntity, new CargoRevealDelay
            {
                RemainingSeconds = revealDuration,
